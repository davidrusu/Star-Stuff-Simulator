...
public class Star_Stuff_Simulator extends PApplet {
...
public void attract(float ax, float ay, float s) {
  int mx = (int) (ax / cellW);
  int my = (int) (ay / cellH);
  for (int x = 0; x < grid.length; x++) {
    for (int y = 0; y < grid[x].length; y++) {
      float dx = ax - x * cellW;
      float dy = ay - y * cellH;
      float d = max(0.1f, sqrt(dx * dx + dy * dy));
      float sd = s/(d*d);
...
public void updateParticles() {
  for (int i = 0; i < particles.length; i++) {
    ...
    float transferRate = 0.5f;//(grid[x][y].dist() - v.dist()) / (grid[x][y].dist() + v.dist());
    float origX = vx;
    float origY = vy;
    Vec g = grid[x][y];
    float dvx = g.x - vx;
...
public int clamp(int l, int u, int c) {
  return min(u, max(l, c));
}

public float clamp(float l, float u, float c) {
  return min(u, max(l, c));
}
...
public void updateBlackHoles() {
  for (int i = 0; i < blackHoles.size(); i++) {
    ...
    Vec g = grid[x][y];
    float force = clamp(0, 0.5f, g.dist());
    
    bhv.addScaled(g.norm(), force * 0.1f);
    bhv.addScaled(width / 2 - bh.x, height / 2 - bh.y, 0.0001f);
    bhv.scale(0.999f); // dampening
    bh.add(bhv);
...
public void updateGrid() {
  ...
  for (int x = 0; x < grid.length; x++) {
    for (int y = 0; y < grid[x].length; y++) {
      ...
      float realD = max(0.000001f, v.dist());
...
class Vec { // mutable because performance :(
  ...
  public void addScaled(Vec v, float s) {
    this.x += v.x * s;
    this.y += v.y * s;
  }

  public void addScaled(float x, float y, float s) {
    this.x += x * s;
    this.y += y * s;
  }
  ...
  public void settings() {  size(displayWidth, displayHeight);  noSmooth(); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--hide-stop", "Star_Stuff_Simulator" };
    if (passedArgs != null) {
